(() => {
  ...
  async function exportAll() {
    setStatus('Preparando exportación completa…');
    try {
      const data = await fetchAllEmployees();
      if (!data.length) return setStatus('No hay empleados para exportar.', 'error');
      const date = new Date().toISOString().slice(0,10);
      saveWorkbook(data.map(exportObject), `nomina_uea_${date}.xlsx`);
      setStatus(`Exportados ${data.length} registros.`, 'success');
